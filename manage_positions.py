...
def list_positions(portfolio_manager: PortfolioManager):
    ...
    print("="*80)
    print(f"{'ID':<3} {'Entry Time':<20} {'Symbol':<6} {'Side':<4} {'Strike':<8} {'Qty':<3} {'Premium':<8} {'Total':<10}")
    print("-"*80)
    
    for i, pos in enumerate(positions, 1):
        entry_time = datetime.fromisoformat(pos.entry_time).strftime("%m/%d %H:%M")
        total_cost = pos.entry_premium * pos.contracts
        print(f"{i:<3} {entry_time:<20} {pos.symbol:<6} {pos.side:<4} ${pos.strike:<7} "
...
def remove_position(portfolio_manager: PortfolioManager, position_id: int):
    ...
    print(f"  {position.side} ${position.strike} x{position.contracts} @ ${position.entry_premium:.2f}")
    
    confirm = input("Are you sure? (y/N): ").strip().lower()
    if confirm == 'y':
        portfolio_manager.remove_position(position)
        print(f"[SUCCESS] Position removed.")
    else:
...
def show_stats(portfolio_manager: PortfolioManager):
    ...
    for symbol, data in symbols.items():
        print(f"  {symbol}: {data['count']} positions, ${data['exposure']:.2f} exposure")
...
def clear_positions(portfolio_manager: PortfolioManager):
    ...
    print(f"[SUCCESS] All positions cleared.")
...
def main():
    ...
    parser.add_argument('command', choices=['list', 'add', 'remove', 'stats', 'clear'],
                       help='Command to execute')
    parser.add_argument('position_id', type=int, nargs='?',
