...
def add_manual_trades():
    ...
    manual_buy_exists = any(
        trade.get('timestamp', '').startswith('2025-08-04') and 
        trade.get('decision') == 'BUY_CALL' and
        trade.get('premium') == '1.42'
        for trade in existing_trades
    )
    
    manual_sell_exists = any(
        trade.get('timestamp', '').startswith('2025-08-04') and 
        trade.get('decision') == 'SELL_CALL' and
        trade.get('premium') == '1.83'
        for trade in existing_trades
    )
    ...
    if new_trades:
        ...
        for trade in new_trades:
            print(f"  {trade['decision']}: {trade['symbol']} ${trade['strike']} {trade['direction']} @ ${trade['premium']}")
...
def view_trade_history():
    ...
    try:
        ...
        trades = df[df['decision'].isin(['BUY_CALL', 'BUY_PUT', 'SELL_CALL', 'SELL_PUT'])].copy()
        ...
        for _, trade in recent_trades.iterrows():
            timestamp = trade['timestamp'][:16] if pd.notna(trade['timestamp']) else 'N/A'
            ...
            print(f"  {timestamp} | {decision} {symbol} ${strike} {direction} @ ${premium}{pnl_str} | {status}")
        ...
        closed_trades = trades[trades['realized_pnl'].notna() & (trades['realized_pnl'] != 0)]
        ...
        if len(closed_trades) > 0:
            ...
            avg_win = closed_trades[closed_trades['realized_pnl'] > 0]['realized_pnl'].mean() if winning_trades > 0 else 0
            avg_loss = closed_trades[closed_trades['realized_pnl'] < 0]['realized_pnl'].mean() if losing_trades > 0 else 0
        ...
        if len(open_positions) > 0:
            ...
            for _, pos in open_positions.iterrows():
                print(f"  {pos['symbol']} ${pos['strike']} {pos['direction']} @ ${pos['premium']}")
        ...
        manual_closed = manual_trades[manual_trades['realized_pnl'].notna() & (manual_trades['realized_pnl'] != 0)]
        if len(manual_closed) > 0:
            ...
            print(f"Manual Trade Performance: ${manual_pnl:.2f} ({manual_win_rate:.1f}% win rate)")
        
        # LLM trade performance
        llm_closed = llm_trades[llm_trades['realized_pnl'].notna() & (llm_trades['realized_pnl'] != 0)]
        if len(llm_closed) > 0:
            ...
            print(f"LLM-Assisted Performance: ${llm_pnl:.2f} ({llm_win_rate:.1f}% win rate)")
