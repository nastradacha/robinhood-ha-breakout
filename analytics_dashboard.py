...
class TradingAnalytics:
    ...
    def __init__(
        self,
        trade_log_path: str = "logs/trade_log.csv",
        ...
        try:
            ...
            if os.path.exists(self.trade_log_path):
                try:
                    ...
                    logger.info(
                        f"Loaded {len(self.trades_df)} trades from {self.trade_log_path}"
                    )
                    ...
                    try:
                        self.trades_df = pd.read_csv(
                            self.trade_log_path, on_bad_lines="skip"
                        )
                        logger.info(
                            f"Loaded {len(self.trades_df)} trades (with some lines skipped) from {self.trade_log_path}"
                        )
                    except Exception as e2:
                        logger.error(
                            f"Failed to parse CSV even with error handling: {e2}"
                        )
                ...
                if not self.trades_df.empty and "timestamp" in self.trades_df.columns:
                    try:
                        ...
                        self.trades_df["timestamp"] = pd.to_datetime(
                            self.trades_df["timestamp"], format="mixed"
                        )
                        logger.info(
                            f"Successfully converted {len(self.trades_df)} timestamps"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Error converting timestamps with mixed format: {e}"
                        )
                        # Fallback: try infer_datetime_format
                        try:
                            self.trades_df["timestamp"] = pd.to_datetime(
                                self.trades_df["timestamp"], infer_datetime_format=True
                            )
                            logger.info(
                                "Successfully converted timestamps with inferred format"
                            )
                        except Exception as e2:
                            logger.error(f"Failed to convert timestamps: {e2}")
                            # Keep timestamps as strings if conversion fails
                            logger.warning(
                                "Keeping timestamps as strings - some analytics may be limited"
                            )
            else:
        ...
        try:
            ...
            for i, line in enumerate(lines[1:], 2):  # Start from line 2
                ...
                if len(fields) == expected_fields:
                    ...
                    logger.warning(
                        f"Line {i} has {len(fields)} fields, expected {expected_fields}. Skipping: {line[:100]}..."
                    )
            ...
            logger.info(
                f"Successfully loaded {len(self.trades_df)} trades from repaired CSV"
            )
    ...
    def calculate_performance_metrics(self) -> Dict:
        ...
        metrics["max_drawdown_pct"] = (
            (max_drawdown / running_max.max() * 100) if running_max.max() != 0 else 0
        )

        # Sharpe ratio (annualized)
        if len(self.trades_df) > 1:
            daily_returns = self.trades_df.groupby(self.trades_df["timestamp"].dt.date)[
                "pnl"
            ].sum()
        ...
        if "option_type" in self.trades_df.columns:
            ...
            metrics["call_win_rate"] = (
                (len(call_trades[call_trades["pnl"] > 0]) / len(call_trades) * 100)
                if len(call_trades) > 0
                else 0
            )
            metrics["put_win_rate"] = (
                (len(put_trades[put_trades["pnl"] > 0]) / len(put_trades) * 100)
                if len(put_trades) > 0
                else 0
            )
        ...
        if self.bankroll_data:
            ...
            starting_bankroll = self.bankroll_data.get(
            ...
            metrics["total_return_pct"] = (
                ((current_bankroll / starting_bankroll - 1) * 100)
    ...
    def generate_performance_report(self, format: str = "cli") -> str:
        ...
        report.append(
            f"Winning Trades:         {metrics.get('winning_trades', 0):,} ({metrics.get('win_rate', 0):.1f}%)"
        )
        ...
        report.append(
            f"Risk-Reward Ratio:      {metrics.get('risk_reward_ratio', 0):.2f}:1"
        )
        report.append("")
        ...
        report.append("-" * 40)
        report.append(
            f"Maximum Drawdown:       ${metrics.get('max_drawdown', 0):,.2f} ({metrics.get('max_drawdown_pct', 0):.1f}%)"
        )
        ...
        if metrics.get("call_trades", 0) > 0 or metrics.get("put_trades", 0) > 0:
            ...
            report.append("-" * 40)
            report.append(
                f"CALL Trades:            {metrics.get('call_trades', 0):,} (Win Rate: {metrics.get('call_win_rate', 0):.1f}%)"
            )
            report.append(
                f"PUT Trades:             {metrics.get('put_trades', 0):,} (Win Rate: {metrics.get('put_win_rate', 0):.1f}%)"
            )
            report.append("")
        ...
        if metrics.get("current_bankroll"):
            ...
            report.append("-" * 40)
            report.append(
                f"Starting Bankroll:      ${metrics.get('starting_bankroll', 0):,.2f}"
            )
            report.append(
                f"Current Bankroll:       ${metrics.get('current_bankroll', 0):,.2f}"
            )
            report.append(
                f"Total Return:           ${metrics.get('total_return', 0):,.2f} ({metrics.get('total_return_pct', 0):+.1f}%)"
            )
            report.append("")
        ...
        report.append("-" * 40)
        report.append(
            f"Overall Grade:          {grade['letter']} ({grade['score']:.1f}/100)"
        )
    ...
    def create_performance_charts(self, output_dir: str = "charts") -> List[str]:
        ...
        cumulative_pnl = self.trades_df["pnl"].cumsum()
        ax.plot(
            range(len(cumulative_pnl)), cumulative_pnl, linewidth=2, color="#2E8B57"
        )
        ax.fill_between(
            range(len(cumulative_pnl)), cumulative_pnl, alpha=0.3, color="#2E8B57"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Trading Performance Analytics Dashboard"
    )
    parser.add_argument(
        "--mode",
        choices=["cli", "web"],
    ...
    parser.add_argument(
        "--export", choices=["html", "csv", "txt"], help="Export report format"
    )
    parser.add_argument(
        "--slack-summary", action="store_true", help="Send performance summary to Slack"
    )
    parser.add_argument(
        "--charts", action="store_true", help="Generate performance charts"
    )
