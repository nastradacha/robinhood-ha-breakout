...
class TestBankrollSeed:
    ...
    def test_bankroll_seeding_logic(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                not bankroll_file.exists()
            ), "Bankroll file should not exist initially"

            # STEP 1: First initialization should seed from YAML START_CAPITAL
            initial_capital = 500.0
            manager1 = BankrollManager(
                str(bankroll_file), start_capital=initial_capital
            )
            ...
            assert (
                data["current_bankroll"] == initial_capital
            ), f"Expected {initial_capital}, got {data['current_bankroll']}"
            assert (
                data["start_capital"] == initial_capital
            ), f"Expected {initial_capital}, got {data['start_capital']}"
            assert (
                data["peak_bankroll"] == initial_capital
            ), f"Expected {initial_capital}, got {data['peak_bankroll']}"
            ...
            manager2 = BankrollManager(
                str(bankroll_file), start_capital=different_capital
            )
            ...
            assert (
                current_bankroll == initial_capital
            ), f"Should ignore new YAML value. Expected {initial_capital}, got {current_bankroll}"
            assert (
                data2["start_capital"] == initial_capital
            ), f"Should ignore new YAML value. Expected {initial_capital}, got {data2['start_capital']}"
            assert (
                data2["peak_bankroll"] == initial_capital
            ), f"Should ignore new YAML value. Expected {initial_capital}, got {data2['peak_bankroll']}"

            # STEP 3: Verify the manager's internal start_capital reflects YAML but doesn't affect persisted data
            assert (
                manager2.start_capital == different_capital
            ), "Manager should store YAML value internally"

            # But the persisted data should remain unchanged
            assert (
                data2["current_bankroll"] == initial_capital
            ), "Persisted data should ignore YAML changes"
    ...
    def test_multiple_load_calls_consistent(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                bankroll1 == bankroll2 == bankroll3 == test_capital
    ...
    def test_file_deletion_recreates_with_yaml(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                manager2.get_current_bankroll() == new_capital
            ), f"Should use new YAML value after file deletion. Expected {new_capital}, got {manager2.get_current_bankroll()}"
