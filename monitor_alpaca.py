...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/monitor_alpaca.log'),
...
class EnhancedPositionMonitor:
    ...
    def get_current_price(self, symbol: str) -> Optional[float]:
        ...
        if self.alpaca.enabled:
            ...
            if price:
                ...
                logger.warning(f"[ALPACA] Failed to get {symbol} price, trying Yahoo...")
    ...
    def estimate_option_price(self, symbol: str, strike: float, option_type: str,
                            expiry: str, current_stock_price: float) -> Optional[float]:
        """
    ...
    def check_position_alerts(self, position: Dict, current_price: float, 
                            estimated_option_price: float) -> None:
        """
        ...
        # Calculate P&L
        current_value = estimated_option_price * quantity * 100  # Options are per 100 shares
        ...
        last_alert_time = self.last_alerts.get(position_key, {}).get('time', datetime.min)
        ...
        if exit_decision.should_exit or exit_decision.reason != ExitReason.NO_EXIT:
            self.handle_exit_decision(position, current_price, estimated_option_price, 
                                    pnl, pnl_pct, exit_decision)
        ...
        # Keep legacy profit level alerts as backup
            ...
            self.check_legacy_alerts(position, current_price, estimated_option_price, 
                                   pnl, pnl_pct, position_key, current_time)
    
    def handle_exit_decision(self, position: Dict, current_price: float, 
                           option_price: float, pnl: float, pnl_pct: float, 
                           exit_decision) -> None:
...
✨ Consider taking profits! ✨
    ...
    def check_legacy_alerts(self, position: Dict, current_price: float, 
                          option_price: float, pnl: float, pnl_pct: float, 
                          position_key: str, current_time: datetime) -> None:
        """Legacy alert system for backward compatibility."""
        # Check profit levels
        for profit_level in self.profit_levels:
            if pnl_pct >= profit_level:
                last_profit_alert = self.last_alerts.get(position_key, {}).get('profit_level', 0)
    ...
    def send_profit_alert(self, position: Dict, current_price: float, 
                         option_price: float, pnl: float, pnl_pct: float, 
                         profit_level: int) -> None:
...
Consider taking profits!
        ...
        logger.info(f"[ALERT] Profit target {profit_level}% hit for {symbol} ${strike} {option_type}")
        print(message)
    
    def send_stop_loss_alert(self, position: Dict, current_price: float,
                           option_price: float, pnl: float, pnl_pct: float) -> None:
...
CONSIDER CLOSING POSITION!
        ...
        logger.warning(f"[ALERT] Stop loss triggered for {symbol} ${strike} {option_type}")
        print(message)
...
Avoid overnight risk!
        ...
        for position in positions:
            try:
                ...
                if not estimated_option_price:
                    logger.error(f"[MONITOR] Could not estimate option price for {symbol}")
                    continue
                
                # Check for alerts
                self.check_position_alerts(position, current_price, estimated_option_price)
                
                # Log current status
                entry_price = position['entry_price']
                pnl_pct = ((estimated_option_price - entry_price) / entry_price) * 100
                
                logger.info(f"[MONITOR] {symbol} ${strike} {option_type}: "
    ...
    def run(self, interval_minutes: int = 1) -> None:
        ...
        """
        logger.info(f"[MONITOR] Starting enhanced monitoring (interval: {interval_minutes}min)")
        logger.info(f"[MONITOR] Data source: {'Alpaca (Real-time)' if self.alpaca.enabled else 'Yahoo (Delayed)'}")
