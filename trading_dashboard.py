...
def load_trade_history() -> List[Dict]:
    ...
    try:
        with open('logs/trade_log.csv', 'r', newline='') as file:
            reader = csv.DictReader(file)
            for row in reader:
                if row.get('action') in ['BUY_CALL', 'SELL_CALL', 'BUY_PUT', 'SELL_PUT']:
                    trades.append(row)
    except FileNotFoundError:
        pass
    except Exception as e:
...
def analyze_trades(trades: List[Dict]) -> Dict:
    ...
    for trade in trades:
        if trade.get('status') == 'CLOSED_PROFIT' or trade.get('status') == 'CLOSED_LOSS':
            try:
...
def display_dashboard():
    ...
    print(f"Total Return: ${current_bankroll - start_capital:+,.2f} ({((current_bankroll - start_capital) / start_capital * 100):+.1f}%)")
    ...
    if positions:
        ...
        for pos in positions:
            ...
            print(f"  {pos['symbol']} ${pos['strike']} {pos['option_type']} x{pos['quantity']}")
        ...
        print(f"Buying Power Used: {(total_invested / (current_bankroll + total_invested) * 100):.1f}%")
    else:
    ...
    if recent_trades:
        for trade in recent_trades:
            ...
            print(f"  {timestamp} | {action} {symbol} ${strike} {option_type} @ ${price}{pnl_str}")
    ...
    print("=== RISK ANALYSIS ===")
    if positions:
        total_at_risk = sum(pos['entry_price'] * pos['quantity'] * 100 for pos in positions)
        risk_percentage = (total_at_risk / (current_bankroll + total_at_risk)) * 100
        print(f"Capital at Risk: ${total_at_risk:,.2f} ({risk_percentage:.1f}% of account)")
    else:
        print("No capital currently at risk")
    
    max_drawdown = start_capital - bankroll_data.get('min_bankroll', start_capital) if 'min_bankroll' in bankroll_data else 0
    if max_drawdown > 0:
        print(f"Max Drawdown: ${max_drawdown:,.2f} ({(max_drawdown / start_capital * 100):.1f}%)")
    else:
    ...
    if trade_analysis['total_closed'] > 0:
        print(f"[OK] {trade_analysis['total_closed']} completed trades with {trade_analysis['win_rate']:.1f}% win rate")
        print(f"[OK] Total profit: ${trade_analysis['total_pnl']:+,.2f}")
        print(f"[OK] Account growth: {((current_bankroll - start_capital) / start_capital * 100):+.1f}%")
    else:
