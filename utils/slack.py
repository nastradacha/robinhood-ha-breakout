...
class SlackNotifier:
    ...
    def __init__(self, webhook_url: Optional[str] = None):
        ...
        if not self.webhook_url:
            self.logger.warning("No Slack webhook URL configured - notifications disabled")
            self.enabled = False
        else:
            self.enabled = True
    ...
    def send_startup_notification(self, dry_run: bool = False) -> bool:
        ...
        color = "#36a64f" if dry_run else "#ff9500"  # Green for dry run, orange for live
    ...
    def send_trade_decision(self, decision: str, confidence: float, reason: str, 
                          bankroll: float, position_size: float = 0) -> bool:
    ...
    def send_order_ready_alert(self, trade_type: str, strike: str, expiry: str, 
                             position_size: float, **kwargs) -> bool:
