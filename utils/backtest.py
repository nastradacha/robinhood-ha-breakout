...
class StrategyBacktester:
    ...
    def __init__(self, config: Dict, use_llm: bool = True):
        ...
        if self.use_llm:
            try:
                ...
                self.logger.warning(f"Failed to initialize LLM: {e}. Using rule-based decisions.")
                self.use_llm = False
    ...
    def fetch_historical_data(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            if days_diff <= 60:
                ...
                interval = "1d"
                self.logger.info("Using daily data for longer backtest (5m data limited to 60 days)")
            ...
            self.logger.info(f"Retrieved {len(data)} bars of {interval} historical data")
            return data
    ...
    def simulate_option_premium(self, underlying_price: float, strike_price: float, 
                              direction: str, dte: int = 0) -> float:
    ...
    def make_trade_decision(self, analysis: Dict, win_history: List[bool]) -> Tuple[str, float, str]:
        ...
        if self.use_llm:
            try:
                ...
                self.logger.warning(f"LLM decision failed: {e}. Using rule-based fallback.")
        ...
        if body_pct < self.min_body_pct:
            return "NO_TRADE", 0.0, f"Candle body {body_pct:.2f}% below minimum {self.min_body_pct}%"
    ...
    def calculate_exit_price(self, entry_premium: float, underlying_entry: float, 
                           underlying_exit: float, direction: str, strike: float) -> float:
        ...
        if direction == "CALL":
            if underlying_change_pct > 0.01:  # Profitable move
                exit_premium = entry_premium * (1 + underlying_change_pct * 3)  # 3x leverage
            else:
    ...
    def run_backtest(self, symbol: str = "SPY", start_date: str = "2024-01-01", 
                    end_date: str = "2024-12-31", initial_capital: float = 10000) -> BacktestResults:
        ...
        for i in range(self.lookback_bars, len(ha_data) - 1):  # Leave room for exit
            ...
            try:
                ...
                # Make trade decision
                decision, confidence, reason = self.make_trade_decision(analysis, win_history[-10:])
                ...
                premium = self.simulate_option_premium(current_price, strike_price, decision)
                ...
                total_cost = (premium * quantity * self.option_multiplier) + (self.commission * quantity)
                ...
                exit_idx = min(i + 12, len(ha_data) - 1)  # Hold for ~1 hour (12 bars of 5min)
                exit_date = ha_data.index[exit_idx]
                exit_price = ha_data.iloc[exit_idx]['Close']
                
                # Calculate exit premium
                exit_premium = self.calculate_exit_price(premium, entry_price, exit_price, decision, strike_price)
                ...
                net_pnl = gross_pnl - (self.commission * quantity * 2)  # Entry + exit commissions
                ...
                self.logger.debug(f"Trade {len(trades)}: {decision} @ ${entry_price:.2f} -> "
        ...
        results = self._calculate_performance_metrics(trades, equity_curve, initial_capital)
    ...
    def _calculate_performance_metrics(self, trades: List[BacktestTrade], 
        ...
        if not trades:
            return BacktestResults(
                ...
                profit_factor=0.0, trades=[], equity_curve=equity_curve,
                daily_returns=[]
            )
    ...
    def generate_report(self, results: BacktestResults, output_file: str = None) -> str:
        ...
        if results.trades:
            ...
            for trade in results.trades[-10:]:
                ...
                report.append(f"{trade.entry_date.strftime('%Y-%m-%d %H:%M')} | "
...
def run_quick_backtest(config_file: str = "config.yaml", days: int = 30) -> BacktestResults:
