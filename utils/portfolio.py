...
class Position:
    ...
    def matches_close_criteria(self, symbol: str, side: str) -> bool:
        ...
        """
        return (self.symbol == symbol and 
                self.side != side)  # Opposite side closes the position
...
class PortfolioManager:
    ...
    def add_position(self, position: Position) -> None:
        ...
        try:
            ...
            logger.info(f"Added position: {position.symbol} {position.side} "
    ...
    def remove_position(self, position_to_remove: Position) -> None:
        ...
        try:
            ...
            for pos in positions:
                if (pos.symbol == position_to_remove.symbol and
                    pos.side == position_to_remove.side and
                    pos.strike == position_to_remove.strike and
                    pos.expiry == position_to_remove.expiry):
                    removed = True
                    logger.info(f"Removing position: {pos.symbol} {pos.side} "
            ...
            logger.info(f"Position removed. {len(remaining_positions)} positions remaining")
    ...
    def find_position_to_close(self, symbol: str, side: str) -> Optional[Position]:
        ...
        for position in positions:
            if position.matches_close_criteria(symbol, side):
                logger.info(f"Found position to close: {position.symbol} {position.side} "
                           f"${position.strike} x{position.contracts}")
                return position
        
        return None
    
    def determine_trade_action(self, symbol: str, side: str) -> Tuple[str, Optional[Position]]:
        """
    ...
    def calculate_realized_pnl(self, position: Position, exit_premium: float) -> float:
        ...
        logger.info(f"Calculated P/L: Entry ${position.entry_premium} -> Exit ${exit_premium} "
    ...
    def log_realized_trade(self, position: Position, exit_premium: float, 
                          realized_pnl: float, trade_log_file: str = "trade_log.csv") -> None:
        """
        ...
        try:
            ...
            logger.info(f"Logged realized trade: {position.symbol} {position.side} "
