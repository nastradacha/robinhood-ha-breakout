...
class AlpacaClient:
    ...
    def __init__(self):
        ...
        if self.enabled:
            try:
                ...
                logger.info(f"[ALPACA] Connected successfully - Account: {account.account_number}")
            ...
            logger.warning("[ALPACA] API keys not configured - falling back to Yahoo Finance")
    ...
    def get_market_data(self, symbol: str, period: str = "1d") -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if period == "1d":
                ...
                start_time = datetime.now() - timedelta(days=7)  # Get extra days for market hours
    ...
    def get_option_estimate(self, symbol: str, strike: float, option_type: str, 
        ...
        try:
            ...
            time_value = self._estimate_time_value(symbol, strike, current_stock_price, expiry)
            
            estimated_price = intrinsic_value + time_value
            estimated_price = max(0.01, estimated_price)  # Minimum $0.01
            
            logger.debug(f"[ALPACA] {symbol} ${strike} {option_type} estimate: ${estimated_price:.2f}")
    ...
    def _estimate_time_value(self, symbol: str, strike: float, 
                           current_price: float, expiry: str) -> float:
        """
        ...
        try:
            ...
            time_value = (0.4 * current_price * volatility * 
                         (time_to_expiry ** 0.5) * atm_factor)
            
            # For 0DTE, time value decays rapidly
            if days_to_expiry == 0:
                time_value *= 0.1  # Very low time value for 0DTE
            
            return max(0.01, min(time_value, current_price * 0.1))  # Cap at 10% of stock price
