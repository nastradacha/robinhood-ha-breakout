...
class SlackChartGenerator:
    ...
    def create_breakout_chart(self, market_data: pd.DataFrame, analysis: Dict, 
                            symbol: str = "SPY") -> str:
        """
        ...
        plt.tight_layout()
        plt.savefig(chart_path, dpi=self.chart_config['dpi'], 
                   facecolor=self.chart_config['facecolor'],
    ...
    def create_position_chart(self, position: Dict, current_price: float, 
                            pnl_pct: float, exit_decision: Dict = None) -> str:
        """
        ...
        Returns:
            Path to generated chart file
        """
        fig, ax = plt.subplots(figsize=(10, 6), facecolor=self.chart_config['facecolor'])
    ...
    def _plot_support_resistance(self, ax, analysis: Dict, data: pd.DataFrame):
        ...
        for level in analysis.get('resistance_levels', [])[:3]:  # Top 3
            ...
            ax.text(data_len * 0.02, level, f'R: ${level:.2f}', 
        ...
        for level in analysis.get('support_levels', [])[:3]:  # Top 3
            ...
            ax.text(data_len * 0.02, level, f'S: ${level:.2f}', 
    ...
    def _plot_current_signal(self, ax, analysis: Dict):
        ...
        if trend == 'BULLISH':
            ax.annotate('📈 BULLISH', xy=(len(ax.get_xlim()) * 0.7, current_price),
                       xytext=(10, 20), textcoords='offset points',
                       fontsize=12, color='#00ff88', weight='bold',
                       arrowprops=dict(arrowstyle='->', color='#00ff88'))
        elif trend == 'BEARISH':
            ax.annotate('📉 BEARISH', xy=(len(ax.get_xlim()) * 0.7, current_price),
    ...
    def _plot_volume_analysis(self, ax, data: pd.DataFrame, analysis: Dict):
        ...
        colors = ['#00ff88' if data.iloc[i]['Close'] >= data.iloc[i]['Open'] 
        ...
        if volume_ratio > 1.5:
            ...
            ax.text(len(volume) * 0.02, volume_avg * 1.5, 'High Volume', 
    ...
    def _plot_pnl_gauge(self, ax, pnl_pct: float, exit_decision: Dict = None):
        ...
        # Color zones
        ax.fill_between(theta, 0, 1, where=(theta <= np.pi/3), 
                       color='red', alpha=0.3, label='Loss Zone')
        ax.fill_between(theta, 0, 1, where=((theta > np.pi/3) & (theta <= 2*np.pi/3)), 
                       color='yellow', alpha=0.3, label='Neutral Zone')
        ax.fill_between(theta, 0, 1, where=(theta > 2*np.pi/3), 
        ...
        ax.arrow(0, 0, 0.8 * np.cos(pnl_angle), 0.8 * np.sin(pnl_angle),
                head_width=0.05, head_length=0.1, fc='white', ec='white')
        
        # P&L text
        ax.text(0, -0.3, f'{pnl_pct:+.1f}%', ha='center', va='center',
    ...
    def _add_position_details(self, ax, position: Dict, current_price: float, pnl_pct: float):
        ...
        for i, detail in enumerate(details):
            ax.text(0.02, 0.98 - i*0.05, detail, transform=ax.transAxes,
    ...
    def _format_main_chart(self, ax, symbol: str, analysis: Dict):
        ...
        ax.set_title(f'{symbol} - Breakout Analysis', 
                    fontsize=self.chart_config['title_size'], 
                    color='white', pad=20)
        
        ax.set_ylabel('Price ($)', fontsize=self.chart_config['label_size'], color='white')
        ax.grid(True, alpha=self.chart_config['grid_alpha'])
        ax.tick_params(colors='white', labelsize=self.chart_config['tick_size'])
        
        # Add analysis summary
        summary = f"Trend: {analysis.get('trend_direction', 'N/A')} | " \
...
class EnhancedSlackNotifier:
    ...
    def send_breakout_alert_with_chart(self, symbol: str, decision: str, analysis: Dict, 
        ...
        try:
            ...
            # Create rich message
            message = self._create_breakout_message(symbol, decision, analysis, confidence)
    ...
    def send_position_alert_with_chart(self, position: Dict, current_price: float, 
        ...
        try:
            ...
            # Create rich message
            message = self._create_position_message(position, current_price, pnl_pct, alert_type)
            
            # Send message with chart
            self._send_message_with_image(message, chart_path)
            
            logger.info(f"[SLACK] Sent position alert with chart for {position['symbol']}")
            
        except Exception as e:
            logger.error(f"[SLACK] Failed to send position alert with chart: {e}")
    
    def _create_breakout_message(self, symbol: str, decision: str, analysis: Dict, confidence: float) -> Dict:
    ...
    def _create_position_message(self, position: Dict, current_price: float, 
                               pnl_pct: float, alert_type: str) -> Dict:
