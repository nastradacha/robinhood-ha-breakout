...
class SlackBot:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("ok"):
                    ...
                    self.logger.error(
                        f"Bot API error: {result.get('error', 'Unknown')}"
                    )
                    return False
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("ok"):
                    ...
                    self.logger.error(
                        f"API error getting messages: {result.get('error')}"
                    )
                    return []
            else:
                self.logger.error(
                    f"HTTP error getting messages: {response.status_code}"
                )
                return []
    ...
    def check_for_trade_confirmations(self, minutes_back: int = 5) -> List[str]:
        ...
        for msg in messages:
            ...
            if any(
                pattern in text
                for pattern in ["filled", "cancelled", "cancel", "submit", "submitted"]
            ):
    ...
    def send_trade_alert(self, trade_details: Dict) -> bool:
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": f"🎯 {direction} Trade Ready"},
            },
            {
                "type": "section",
                "fields": [
                    {"type": "mrkdwn", "text": f"*Strike:* ${strike}"},
                    {"type": "mrkdwn", "text": f"*Premium:* ${premium:.2f}"},
