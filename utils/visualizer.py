...
class TradingVisualizer:
    ...
    def create_beginner_summary(self, results, save_path: str = None) -> str:
        ...
        ax1.text(
            0,
            total_pnl + (abs(total_pnl) * 0.1 if total_pnl != 0 else 100),
            f"${total_pnl:,.2f}",
            ha="center",
        ...
        ax3.text(
            0,
            results.total_trades + max(1, results.total_trades * 0.05),
        ...
        plt.suptitle(
            "Your Trading Results - Easy Summary",
            fontsize=18,
            fontweight="bold",
            y=0.98,
        )
        ...
        if save_path is None:
            save_path = (
                self.output_dir
                / f"beginner_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )
    ...
    def create_equity_curve(self, results, save_path: str = None) -> str:
        ...
        ax.fill_between(
            dates, results.equity_curve, alpha=0.3, color=self.colors["bullish"]
        )
        ...
        ax.set_title(
            "📈 Your Portfolio Value Over Time", fontsize=16, fontweight="bold"
        )
    ...
    def create_trade_analysis(self, results, save_path: str = None) -> str:
        ...
        colors = [
            self.colors["profit"] if pnl > 0 else self.colors["loss"] for pnl in pnls
        ]
...
if __name__ == "__main__":
    ...
    print(
        "Use create_all_visualizations() to generate charts for your backtest results."
    )
